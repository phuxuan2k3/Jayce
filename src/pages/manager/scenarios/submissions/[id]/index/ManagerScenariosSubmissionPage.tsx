...
const ScenarioSubmissionDetail = () => {
  ...
  const handleGoToSubmissionListView = () => {
    navigate(paths.manager.scenario.in(scenarioInfo.id).SUBMISSIONS, {
      state: { scenarioId: scenarioInfo.id },
    });
  };
        ...
        <div className="w-full max-w-7xl py-6">
          ...
          <div className="flex flex-col items-center">
            ...
            {attempt.answers.map((answer, index) => (
              <div
                key={index}
                className="w-4/6 flex-1 flex flex-row bg-white rounded-lg shadow-primary p-6 space-x-2 border-r border-b border-solid border-primary justify-between mb-4"
              >
                ...
                <div className="w-7/8 flex flex-col items-start justify-start">
                  <span className="font-bold mb-2 opacity-50">Rating</span>
                  <span className="font-bold mb-2 text-primary">
                    Relevance:{" "}
                    <span className="bg-[#EAF6F8]  border-2 border-primary rounded-lg pl-1 pr-1">
                      {answer.relevance}/10
                    </span>
                  </span>
                  <span className="font-bold mb-2 text-primary">
                    ...
                    <span className="bg-[#EAF6F8]  border-2 border-primary rounded-lg pl-1 pr-1">
                      {answer.clarity_completeness}/10
                    </span>
                  </span>
                  <span className="font-bold mb-2 text-primary">
                    Accuracy:{" "}
          ...
          <div className="flex flex-row justify-center">
            <button
              ...
              onClick={handleGoToSubmissionListView}
            >
