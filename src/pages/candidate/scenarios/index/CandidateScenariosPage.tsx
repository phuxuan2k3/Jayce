...
} from "../../../../features/scenarios/apis/concrete/ekko.scenario-api";
...
const CandidateScenariosPage = () => {
  ...
  const [dates, setDates] = React.useState({
    from: new Date("2000-01-01").toISOString().split("T")[0],
  ...
  const [sortType, setSortType] = React.useState<
    "participants" | "rating" | "created_at" | null
  >(null);
  const [minRating, setMinRating] = React.useState<number>(0);
  const [minParticipant, setMinParticipant] = React.useState<number>(0);
  const [filterForFetch, setFilterForFetch] = React.useState<{
    minRating: number;
  ...
  const [scenarios, setScenarios] = React.useState<
    (Scenario & { attempted: boolean; hasFetched: boolean })[]
  >([]);

  const handlePractice = (scenarioId: number) => {
    navigate(paths.candidate.scenarios.in(scenarioId)._layout, {
      state: { scenarioId },
    });
  };
    ...
    const fetchScenarios = async () => {
      try {
        const response = await listScenarioMutation({
          bm_ids: [],
          sort_methods: sortType
            ? [{ name: sortType, type: SortType.SORT_TYPE_DESC }]
            : [],
  ...
  React.useEffect(() => {
    ...
    if (
      scenarios.length > 0 &&
      scenarios.some((scenario) => !scenario.hasFetched)
    ) {
      fetchAttempts();
    }
  }, [scenarios]);
  ...
  return (
    <>
      <div className="mx-12 font-arya">
        ...
        <div className="mt-2 text-[var(--primary-color)]">
          You can practice interview skills with multiple situations
        </div>
        <div className=" flex justify-between">
          <div className="flex gap-3 mt-8">
            <div
              className={`rounded-lg bg-[var(--primary-color)] px-4 py-2 font-bold text-white cursor-pointer ${sortType === "participants" ? "text-secondary" : ""}`}
              onClick={() =>
            ...
            <div
              className={`rounded-lg bg-[var(--primary-color)] px-4 py-2 font-bold text-white cursor-pointer ${sortType === "rating" ? "text-secondary" : ""}`}
              onClick={() =>
                sortType === "rating"
                  ? setSortType(null)
                  : setSortType("rating")
            ...
            <div
              className={`rounded-lg bg-[var(--primary-color)] px-4 py-2 font-bold text-white cursor-pointer ${sortType === "created_at" ? "text-secondary" : ""}`}
              onClick={() =>
                sortType === "created_at"
                  ? setSortType(null)
            ...
            <div
              className="rounded-lg bg-[var(--primary-color)] px-4 py-2 font-bold text-white flex items-center cursor-pointer"
          ...
          <div className="flex gap-6">
            <label className="inline-flex items-center">
              <input
                type="checkbox"
                className="h-6 w-6 border-2 border-gray-400 checked:bg-green-500 checked:border-green-500 cursor-pointer"
          ...
          {scenarios.map((scenario, index) => {
            ...
            return (
              <div key={index} className="bg-gray-100 p-4 rounded-lg mb-4 ">
                <div
                  className="text-xl font-bold mb-3 cursor-pointer hover:text-primary"
                ...
                <div className="mt-4 flex justify-between items-center text-gray-500">
                  <div className="flex gap-10 items-center">
                    <div className="text-sm flex gap-2">
                      {Number.isInteger(scenario.rating)
                        ? scenario.rating
                        : scenario.rating.toFixed(2)}{" "}
                      <FaStar className="text-gray-500" />
                    </div>
                    <div className="text-sm flex gap-2">
                      {scenario.total_participant} <FaUser />
                    </div>
                    <div className="text-sm flex gap-2">
                    ...
                    <div
                      className={`px-3 py-1 rounded-lg text-sm font-medium ${scenario.attempted ? "bg-[var(--primary-color)] text-white" : "text-gray-500 border border-gray-500"}`}
                    >
                      {scenario.attempted ? "Attempted" : "Not Attempted"}
                    </div>
                  </div>
                  <div
                    className="rounded-lg bg-[var(--primary-color)] py-1 px-4 font-bold text-white flex gap-2 items-center cursor-pointer"
                    onClick={() => handlePractice(scenario.id)}
                  >
                    <FaArrowRight /> Practice
                  </div>
                </div>
              </div>
            );
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 font-arya">
          <div className="bg-gray-100 p-6 rounded-lg w-100 shadow">
            ...
            <div className="flex flex-col gap-4 w-full">
              ...
              <label className="grid grid-cols-3 items-center justify-between gap-2">
                ...
                <input
                  ...
                  min={0}
                  onChange={(e) =>
                    setMinRating(Math.min(Number(e.target.value), 5))
                  }
                />
              </label>
              <label className="grid grid-cols-3 items-center justify-between gap-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDates({
                      ...dates,
                      to: new Date(e.target.value).toISOString().split("T")[0],
                    })
                  }
                />
              </label>
            ...
            <div className="flex justify-between gap-3 mt-4 mx-12">
              ...
              <button
                className="px-4 py-2 bg-[var(--primary-color)] text-white rounded-lg cursor-pointer"
