...
const Settings = () => {
  ...
  const [selectedTab, setSelectedTab] = React.useState<
    "Public profile" | "Account" | "Theme preferences" | "Notifications"
  >("Public profile");
  ...
  const [authBalance, setAuthBalance] = React.useState<{
    balance: number;
    is_premium: boolean;
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-6 pl-4 pr-4 pt-8 pb-8 lg:pl-12 lg:pr-12">
        ...
        <div className="w-full flex flex-col lg:flex-row gap-4">
          <div className="w-full lg:w-[18%]">
            <div className="flex flex-row lg:flex-col gap-2 overflow-x-auto">
              ...
              <div
                ...
                className="flex-shrink-0 p-2 rounded cursor-pointer text-center lg:text-start"
              >
                <SubtitlesOutlinedIcon className="h-6 w-6 mr-1 opacity-50" />{" "}
                <span className="opacity-50">{t("settings_tab_billing")}</span>{" "}
                <OpenInNewOutlinedIcon className="text-primary h-4 w-4 ml-1" />
              </div>
            </div>
          </div>
          ...
          <div className="w-full lg:w-[80%]">
            {selectedTab === "Public profile" && (
              <PublicProfile authData={authData} />
            )}
      ...
      >
        ...
        {creatingError && (
          <div className="text-center text-red-500 mb-4">{creatingError}</div>
        )}
        <DialogActions className="flex items-center justify-between px-6 pb-6">
          ...
          >
            {isCreating
              ? t("settings_topup_processing")
              : t("settings_topup_confirm")}
          </button>
        </DialogActions>
      </Dialog>
